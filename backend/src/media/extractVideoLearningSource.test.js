...
test("full ASR mode waits for and merges every audio chunk", async () => {
  const completed = [];
  const learningSource = await extractVideoLearningSource({
    sourceUrl: "https://www.bilibili.com/video/BV1fullasr",
    asrMode: "full",
    publicMediaBaseUrl: "https://api.example.com",
    provider: {
      fetchVideoSource: async () => ({
      ...
      transcribeMedia: async ({ mediaUrl }) => {
        ...
        return {
          ...
          segments: [{
            ...
            startSeconds: 1,
            endSeconds: 9,
            text: `全片分片${index + 1}的完整语音内容，包含清晰观点、事实依据、过程解释和可以用于复习的具体结论。`
          }]
        };
      },
      transcribeAudio: async () => { throw new Error("local fallback should not run"); }
  ...
  assert.equal(learningSource.extractionMeta.asr.mode, "full");
  assert.equal(learningSource.extractionMeta.asr.sampled, false);
  assert.equal(learningSource.extractionMeta.asr.coverage.completedChunks, 4);
  assert.equal(learningSource.extractionMeta.asr.coverage.totalChunks, 4);
  assert.equal(learningSource.extractionMeta.asr.coverage.ratio, 1);
  assert.deepEqual(learningSource.extractionMeta.asr.coverage.temporalRegions.sort(), ["end", "front", "middle"]);
  assert.equal(learningSource.extractionMeta.asr.coverage.frontMiddleEndCovered, true);
  assert.deepEqual(learningSource.extractionMeta.asr.coverage.usedChunks.map((chunk) => chunk.index), [1, 2, 3, 4]);
  assert.equal(learningSource.transcriptSegments.length, 4);
});

test("sampled ASR covers at least 30 percent plus front, middle, and end", async () => {
  const restoreEnv = setEnvForTest({
    LOCAL_ASR_PARALLEL_THRESHOLD_SECONDS: "1",
    LOCAL_ASR_MAX_CHUNKS: "12",
    LOCAL_ASR_SUCCESS_QUORUM: "5",
    LOCAL_ASR_CHUNK_CONCURRENCY: "6"
  });
  try {
    const learningSource = await extractVideoLearningSource({
      sourceUrl: "https://www.bilibili.com/video/BV1sampledcoverage",
      asrMode: "sampled",
      provider: {
        fetchVideoSource: async () => ({
          provider: "bilibili_api",
      ...
      downloadMedia: async () => ({
        path: "/tmp/video-dir/source.mp4",
        dir: "/tmp/video-dir",
        contentType: "video/mp4"
      }),
      splitAudioForAsr: async () => ({
        dir: "/tmp/video-dir/asr-chunks",
        chunks: Array.from({ length: 70 }, (_, index) => ({
          path: `/tmp/chunk-${index}.mp3`,
          contentType: "audio/mpeg",
          chunkIndex: index,
      ...
      speechToTextProvider: { name: "local_whisper" },
      transcribeAudio: async ({ audioPath }) => {
        const index = Number(String(audioPath).match(/chunk-(\d+)/)?.[1] || 0);
        if (index >= 4) await new Promise((resolve) => setTimeout(resolve, index >= 8 ? 35 : 20));
        return {
          ...
          segments: [{
            ...
            startSeconds: 0,
            endSeconds: 8,
            text: `抽样分片${index + 1}包含足够清晰的事实、解释、上下文和可以生成复习题目的完整观点。`
          }]
        };
      },
      cleanup: async () => {}
    ...
    const coverage = learningSource.extractionMeta.asr.coverage;
    assert.equal(learningSource.extractionMeta.asr.mode, "sampled");
    assert.ok(coverage.completedChunks >= 21);
    assert.equal(coverage.sourceTotalChunks, 70);
    assert.ok(coverage.sourceChunkRatio >= 0.3);
    assert.equal(coverage.targetSourceCoverageRatio, 0.3);
    assert.equal(coverage.frontMiddleEndCovered, true);
    assert.deepEqual([...coverage.temporalRegions].sort(), ["end", "front", "middle"]);
    assert.ok(coverage.usedChunks.some((chunk) => chunk.region === "front"));
    assert.ok(coverage.usedChunks.some((chunk) => chunk.region === "middle"));
    assert.ok(coverage.usedChunks.some((chunk) => chunk.region === "end"));
  } finally {
    restoreEnv();
  }
});
