...
- `RecallRoundView` 管理本轮最多十张卡片的视觉与交互；自评仍调用 `OmoStore.assess` 和现有 assessment API。
- `RecallInteractionState` 只描述可测试的本地交互状态，不改变 `MemoryCard`、API、调度或服务端幂等语义。
- `RecallDesign` 集中保存页面 Palette、402 × 874 参考画布、卡片与滑杆 Metrics；SwiftUI View 不散落 Figma 坐标。

## 首页与抽取

- 有到期卡时，IP 是开始本轮的唯一主入口；箭头只用于暗示点击 IP。
- 点击后取 `dueCards.prefix(10)`，因此候选不足十张时自然抽取全部。
...
- 主卡显示真实 `recallCue`；刮层覆盖真实 `answer`，不展示原截图。
- 答案文字由 Canvas 绘制，揭示前不会进入 VoiceOver 树。
- 手势轨迹按网格计算覆盖率；小于 80% 时只局部揭示且不显示自评。
- 达到 80% 后自动完整揭示，并提供一次轻触觉与 VoiceOver 宣告。
- 揭示后的答案使用粗体和珊瑚橙，问题使用常规字重和深青色，以颜色和字重建立视觉权重。
- 不显示“直接揭晓”等额外文字按钮。
...
- 右下上传入口直接打开系统单选照片选择器。
- 当前卡右上入口暂时复用 Library 完整内容入口；独立的“完整上下文”页面不属于本次实现范围。

## 可访问性与动画

- 菜单、IP、收藏夹、上传和卡片次级入口的触控区域不小于 44pt，并提供 label/hint。
- 揭示前无障碍树不包含 `answer`；揭示后朗读真实答案。
