...
- 做题是首页主体上的状态变化，不是覆盖首页的独占页面；收藏夹与上传入口在待机态和做题态都保持可见、可操作。进入知识库后可通过其返回入口回到首页；当前 MVP 不另造一个“退出本轮”按钮。
...
拖动跨入节点时触发轻震。轨道先定义一条固定在全长坐标中的奶油色到品牌青绿色渐变，拖动只揭示起点至滑块当前位置的部分，不把同一渐变重新压缩到当前长度；因此轨道上同一位置的颜色始终不变，滑块描边显示其所在位置对应的渐变颜色，内部箭头固定使用品牌青绿色、不参与渐变。三个结果不使用红黄绿等离散状态色，避免把诚实自评暗示成由差到好的评分。在任一节点松开即提交；拖回最左取消区松开则回到起点、不提交、不切卡。提交期间锁定滑杆，失败时保留当前揭示状态并提供原地重试，成功后自动进入下一张。

## 次级入口

- 左上菜单包含 Profile 和 Settings；菜单打开时底层页面从无障碍树隐藏。
- 左下收藏夹在待机和做题期间都可进入 Library；库内知识卡直接显示完整内容，不使用刮层。
- 右下上传入口在待机和做题期间都可直接打开系统单选照片选择器；做题期间新上传的卡不会插入当前已经抽出的牌组。
