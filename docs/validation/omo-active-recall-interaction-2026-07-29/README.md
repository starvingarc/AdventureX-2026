...
- 三个自评结果统一使用品牌青绿色；拖动只改变位置和填充长度，不出现红黄绿的价值暗示。
- 自评轨道使用固定的全长渐变坐标；滑块移动只揭示对应区间，滑块描边取当前位置颜色，内部箭头保持固定品牌青绿色。
...
- `iphone17-fixed-gradient-midpoint.png`：滑块停在中间节点时，轨道显示固定全长渐变的前半段，箭头不参与渐变。
