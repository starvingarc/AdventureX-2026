...
- [ ] 对比最新 Omo 主线、Figma 原型与迁移原型，列出只属于 UI 的可复用差异。
- [ ] 复现首次上传首页、首页公共脚手架与资源，并补齐空态测试。
- [ ] 复现有卡首页、十连抽卡堆、刮层、揭示阈值和三节点自评，并适配最新状态机。
- [ ] 检查揭示前不泄露 `hiddenSemantic`，补齐取消、确认、换卡和忘记重封测试。
- [ ] 更新稳定文档和素材来源，运行文档与静态门禁。
- [ ] 构建并在 Simulator 检查核心页面、交互、VoiceOver 与轻量尺寸边界。
...
- 下一位 Agent 从哪里继续：先读取本计划与 `git status`，再对比 `omo/main` 和 `/Users/hanmingyu/Documents/Recallo2.0/Omo-sync`，只迁移计划范围内差异。
