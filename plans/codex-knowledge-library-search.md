...
- [ ] 写入经确认的产品设计规格和逐步实施计划，并完成自审。
- [ ] 补齐知识库 PRD：用户故事、范围、状态机、文字/语音搜索、分页、无障碍、数据与埋点边界。
- [ ] 以测试先行定义搜索协议、请求取消、mock 结果、无结果和失败状态。
- [ ] 以测试先行定义按测量高度分页的纯布局模型，覆盖不同卡片长度、容器高度和 Dynamic Type 缩放。
